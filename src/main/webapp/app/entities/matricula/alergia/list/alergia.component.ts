...
export class AlergiaComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.alergiaService
      ...
      .subscribe({
        next: (res: HttpResponse<IAlergia[]>) => {
  ...
  trackId(_index: number, item: IAlergia): number {
    return item.id!;
  }

  delete(alergia: IAlergia): void {
    const modalRef = this.modalService.open(AlergiaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.alergia = alergia;
  ...
  protected onSuccess(data: IAlergia[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
