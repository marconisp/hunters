...
public interface DadosPessoaisRepository extends JpaRepository<DadosPessoais, Long> {
    ...
    @Query(
        value = "select distinct dadosPessoais from DadosPessoais dadosPessoais left join fetch dadosPessoais.estadoCivil left join fetch dadosPessoais.raca left join fetch dadosPessoais.religiao",
    ...
    @Query(
        "select distinct dadosPessoais from DadosPessoais dadosPessoais left join fetch dadosPessoais.estadoCivil left join fetch dadosPessoais.raca left join fetch dadosPessoais.religiao"
    )
    List<DadosPessoais> findAllWithToOneRelationships();

    @Query(
        "select dadosPessoais from DadosPessoais dadosPessoais left join fetch dadosPessoais.estadoCivil left join fetch dadosPessoais.raca left join fetch dadosPessoais.religiao where dadosPessoais.id =:id"
    )
